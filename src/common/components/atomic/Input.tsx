        ...
        <div className={`flex flex-col gap-2 ${className}`}>
            {label && (
                <div className='flex gap-2 items-center text-sm'>
                    {tooltip && (
                        <>
                            <InformationCircleIcon className='text-secondary w-5 h-5' data-tooltip-id="tooltip"/>
            ...
            <div className={`w-full h-full ${type !== "file" && 'relative'}`}>
                {type != "file" && Icon && (
                    <div
                        className={`absolute flex ${type === 'textarea' ? 'py-3' : 'items-center'} inset-y-0 left-0 pl-4`}>
                        <Icon
                            className={`w-5 h-5 transition-colors duration-200 ${disabled ? 'text-gray-400' : 'text-primary'}`}/>
                    </div>
                )}
                {type === 'select' && (
                    <select
                ...
                {type === 'textarea' && (
                    <textarea
                        ...
                        required={disabled != undefined ? disabled : (required != undefined ? required : true)}
                        disabled={disabled}
                ...
                {type !== 'select' && type !== 'textarea' && type !== 'checkbox' && (
                    <input
                        ...
                        accept={(() => {
                            ...
                            if (accept === 'doc') return 'application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document'
                            return undefined
                        })()}
                        required={required != undefined ? required : true}
                        disabled={disabled}
