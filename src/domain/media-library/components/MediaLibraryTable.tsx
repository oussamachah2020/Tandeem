...
const MediaLibraryTable: FC<Props> = ({mediaLibrary, onUpdate, canDelete}) => {
    const {confirmation} = useStaticValues()
    return (
        <Datatable isEmpty={mediaLibrary.length === 0} headers={['Titre', 'Description']}>
            {mediaLibrary.map((media, idx) => (
                ...
                >
                    ...
                    <DatatableValue className='w-3/4 text-sm leading-relaxed'>{media.description}</DatatableValue>
                </DatatableRow>
            ))}
        </Datatable>
    );
